...
while running and not board.is_game_over():
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if board.turn == chess.WHITE:  # Human player's turn
                mouse_pos = pygame.mouse.get_pos()
                file = mouse_pos[0] // SQUARE_SIZE
                rank = 7 - mouse_pos[1] // SQUARE_SIZE  # Flip rank because chessboard is drawn from top to bottom
    ...
    for square in chess.SQUARES:
        ...
        if piece is not None:
            piece_img = pygame.image.load(f"pieces/{piece.symbol()}.png").convert_alpha()
            screen.blit(piece_img, (chess.square_file(square) * SQUARE_SIZE, (7 - chess.square_rank(square)) * SQUARE_SIZE))

    # Highlight selected square
    if selected_square is not None:
        highlight_rect = pygame.Rect(chess.square_file(selected_square) * SQUARE_SIZE,
